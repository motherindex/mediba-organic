...
export function orderConfirmedEmail({ customerName, orderId, amountTotal, shippingCost, shippingAddress }: {
  customerName: string; orderId: string; amountTotal: number; shippingCost: number; shippingAddress: any;
}) {
  ...
  return emailWrapper(`
    ...
    <p style="color:#7A6A5A;font-size:14px;line-height:1.7;margin:0 0 24px;">Hi ${customerName}, thank you for your order! We're getting it ready.</p>
    ...
    <p style="color:#7A6A5A;font-size:13px;line-height:1.7;margin:0;">We'll send you another email as soon as your order ships. 🌿</p>
  `);
}

export function orderShippedEmail({ customerName, orderId, trackingNumber, carrier }: {
  customerName: string; orderId: string; trackingNumber?: string; carrier?: string;
}) {
  return emailWrapper(`
...
export function orderDeliveredEmail({ customerName, orderId }: { customerName: string; orderId: string }) {
  return emailWrapper(`
    ...
    <p style="color:#7A6A5A;font-size:14px;line-height:1.7;margin:0 0 24px;">Hi ${customerName}, your Mediba's Organic order has been delivered. We hope you enjoy it!</p>
    <div style="background:#fff;border:1px solid #E8DFD0;border-radius:6px;padding:20px;margin-bottom:24px;">
      <p style="margin:0 0 8px;font-size:13px;color:#9E8B7A;">Order ID</p>
      <p style="margin:0;font-size:14px;color:#3B2A1A;font-weight:600;">#${orderId.slice(0, 8).toUpperCase()}</p>
    </div>
    <p style="color:#7A6A5A;font-size:13px;line-height:1.7;margin:0;">Loved your experience? Share Mediba's Organic with a friend. 🌿</p>
  `);
}
...
export function orderCancelledEmail({ customerName, orderId }: { customerName: string; orderId: string }) {
  return emailWrapper(`
    ...
    <p style="color:#7A6A5A;font-size:14px;line-height:1.7;margin:0 0 24px;">Hi ${customerName}, your order has been cancelled. If this was a mistake, please contact us.</p>
    <div style="background:#fff;border:1px solid #E8DFD0;border-radius:6px;padding:20px;margin-bottom:24px;">
      <p style="margin:0 0 8px;font-size:13px;color:#9E8B7A;">Order ID</p>
      <p style="margin:0;font-size:14px;color:#3B2A1A;font-weight:600;">#${orderId.slice(0, 8).toUpperCase()}</p>
    </div>
    <p style="color:#7A6A5A;font-size:13px;line-height:1.7;margin:0;">Contact us at <a href="mailto:medibaorganic@gmail.com" style="color:#C4924A;">medibaorganic@gmail.com</a> and we'll sort it out. 🌿</p>
  `);
}