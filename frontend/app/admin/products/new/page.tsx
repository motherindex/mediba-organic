...
export default function NewProductPage() {
  ...
  async function handleSubmit(e: React.SyntheticEvent<HTMLFormElement>) {
    ...
    setSuccessMessage("Product created! Redirecting…");
    setTimeout(() => {
      ...
      <div style={{ maxWidth: 720, margin: "0 auto" }}>
        ...
        <form onSubmit={handleSubmit} noValidate className="admin-form-card">
          <div style={F.fieldGroup}>
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Raw Shea Butter 250g" style={F.input} disabled={loading || !!successMessage} />
          </div>

          <div style={F.fieldGroup}>
            <label style={F.label}>Description <span style={{ color: "var(--gold)" }}>*</span></label>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Describe the product…" style={F.textarea} disabled={loading || !!successMessage} />
          </div>

          <div style={F.fieldGroup}>
            <label style={F.label}>Price (USD) <span style={{ color: "var(--gold)" }}>*</span></label>
            <input type="number" step="0.01" min="0.01" value={price} onChange={(e) => setPrice(e.target.value)} placeholder="0.00" style={F.input} disabled={loading || !!successMessage} />
          </div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 8 }}>
            <button type="submit" disabled={loading || !!successMessage} className="btn-primary" style={{ opacity: loading || successMessage ? 0.7 : 1 }}>
              {loading ? "Saving…" : "Save Product"}
            </button>
            <button type="button" onClick={() => router.push("/admin/products")} className="btn-outline" disabled={loading}>
              Cancel
            </button>
          </div>
        </form>
